...
def build(day: date) -> tuple[str, str, str]:
    ...
    parts.append('<p class="note">По Внукову и Домодедову разреза по терминалам нет: '
                 'там терминал совпадает с зоной, всё видно в таблице выше.</p>')
    if unknown_planned:
    ...
    if any(t == "н/д" for (_, _, t) in by_full):
        parts.append('<p class="note">Там, где строка без терминала, источник его '
                     ...
                     'только по зонам.</p>')
    parts.append('<p class="note"><b>Как читать задержки.</b> Аэропорты сообщают '
                 'фактическое время вылета по-разному, и на мелких задержках их '
                 'между собой сравнивать нельзя. Шереметьево отдаёт отход от гейта. '
                 'Внуково отдаёт время, похожее на отрыв от полосы, там внутри сидит '
                 'руление, поэтому его цифра завышена. Домодедово публикует новое '
                 'время только когда рейс уже опаздывает, и не показывает задержки '
                 'длиннее двух часов, поэтому его цифра занижена. На пороге в один '
                 'час эти различия почти исчезают: проверка по Шереметьеву, где есть '
                 'оба времени, дала расхождение 2.8 процентных пункта против 40.5 на '
                 'пороге в 15 минут. Надёжнее всего смотреть каждый аэропорт против '
                 'его собственного среднего в соседнем столбце.</p>')
